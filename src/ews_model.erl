...
get_elem(#elem{qname=Key}, Table) ->
    get_elem(Key, Table);
%% Key is a qname: {Namespace, Name}, or a bare Name for an element that is
%% not namespace-qualified.
get_elem(Key, Table) when is_list(Key); tuple_size(Key) == 2 ->
...
get_elem(Key, Parent, Table) when is_list(Key); tuple_size(Key) == 2 ->
