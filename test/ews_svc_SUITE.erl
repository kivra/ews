...
empty_lists_services(_Config) ->
    {ok, []} = ews_svc:list_services().

empty_list_types(_Config) ->
    [] = ews_svc:list_types().

empty_get_model(_Config) ->
    undefined = ews_svc:get_model().

empty_list_full_clashes(_Config) ->
    [] = ews_svc:list_full_clashes().

empty_list_simple_clashes(_Config) ->
    [] = ews_svc:list_simple_clashes().

empty_emit_model(_Config) ->
    {error, no_model} = ews_svc:emit_model("test_file").
...
one_model_list_services(_Config) ->
    {ok, ["CampaignService"]} = ews_svc:list_services().

one_model_list_service_ops(_Config) ->
    Ops = ["get","mutate","query"],
    {ok, Ops} = ews_svc:list_service_ops("CampaignService").
...
one_model_mutate_op_info(_Config) ->
    ...
    {ok, MutateInfo} = ews_svc:get_op_info(Service, "mutate"),
...
one_model_query_op_info(_Config) ->
    ...
    {ok, QueryInfo} = ews_svc:get_op_info(Service, "query"),
...
one_model_get_op(_Config) ->
    Service = "CampaignService",
    {ok, Op} = ews_svc:get_op(Service, "get"),
    "get" = Op#op.name.

one_model_mutate_op(_Config) ->
    Service = "CampaignService",
    {ok, Op} = ews_svc:get_op(Service, "mutate"),
    "mutate" = Op#op.name.

one_model_query_op(_Config) ->
    Service = "CampaignService",
    {ok, Op} = ews_svc:get_op(Service, "query"),
    "query" = Op#op.name.

one_model_list_types(_Config) ->
    95 = length(ews_svc:list_types()).
...
one_model_emit_model(_Config) ->
    ...
    {model_emitted, "test_file"} = ews_svc:emit_model("test_file"),
